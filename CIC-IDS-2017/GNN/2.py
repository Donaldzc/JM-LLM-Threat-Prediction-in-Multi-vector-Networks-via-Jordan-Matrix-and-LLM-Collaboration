...
def evaluate(model, data, num_classes):
    ...
    with torch.no_grad():
        ...
        if num_classes == 2:
            ...
            # 多分类问题
            y_true = label_binarize(true_labels[data.test_mask.cpu().numpy()], classes=range(num_classes))
            y_score = out[data.test_mask.cpu().numpy()].cpu().numpy()
...
# 主函数
def main():
    X, y, df, num_classes = load_cic_ids_2017(r'D:\code\lunwen\data\test.csv')
    ...
    if num_classes == 2:
        key_node_scores = model_output[:, 1].detach

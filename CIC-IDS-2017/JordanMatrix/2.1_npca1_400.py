...
def load_cic_ids_2017(data_path):
    ...
    # 检查标签的取值范围
    print(f"标签最小值: {y.min()}, 标签最大值: {y.max()}")
...
def main():
    X, y, df = load_cic_ids_2017(r'D:\code\lunwen\data\merged.csv')
    num_classes = len(np.unique(y))  # 根据实际标签数量调整分类数量
    print(f"分类数量: {num_classes}")

    data = build_graph_data(X, y, df, k_neighbors=3)  # 进一步减少k值
    ...
    for n_components in range(50, 401, 10):
        ...
        model = CollaborativeAnalysisModel(
            ...
            num_classes=num_classes  # 根据实际标签数量调整分类数量
        ).to(device)
        ...
        results.append({
            ...
            'Test Acc': final_metrics["test_acc"],
            'RPA': final_metrics["rpa"]
        })
        print(f'n_components {n_components},Loss {loss},Train Acc {metrics["train_acc"]},Val Acc:{ metrics["val_acc"]},Test Acc: {final_metrics["test_acc"]},RPA: {final_metrics["rpa"]}')

    # 打印记录结果
    for result in results:
        print(f"n_components: {result['n_components']}, Loss: {result['Loss']:.4f}, Train Acc: {result['Train Acc']:.4f}, "
              f"Val Acc: {result['Val Acc']:.4f}, Test Acc: {result['Test Acc']:.4f}, RPA: {result['RPA']:.4f}")

...
class ThreatPropagationPredictor:
    def __init__(self, model, data):
        ...
        for i in range(edge_index.shape[1]):
            ...
            self.adj_matrix[src, dst] = edge_attr[i]
        ...
        for step in range(steps):
            if method == 'random_walk':
                ...
                for i, infected_idx in enumerate(initial_infected_nodes):
                    # 使用余弦相似度
                    sim = np.dot(self.node_embedding, self.node_embedding[infected_idx]) / (
                        np.linalg.norm(self.node_embedding, axis=1) * 
                        np.linalg.norm(self.node_embedding[infected_idx]) + 1e-10
                    )
                    similarity[i] = sim
            ...
            if step < steps - 1:
                ...
                # 只添加新感染的节点
                initial_infected_nodes = np.unique(np.concatenate([initial_infected_nodes, new_infected_nodes]))
...
def main():
    ...
    print(f'Potential victims after 5 steps: {potential_victims}')
    print(f'Infection probabilities: {final_step[potential_victims]}')

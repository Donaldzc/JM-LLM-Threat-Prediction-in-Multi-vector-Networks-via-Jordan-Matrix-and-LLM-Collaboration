...
class JordanMatrixDecomposition:
    def __init__(self, adj_matrix, n_components=100, solver='lobpcg', max_iter=30000, tol=1e-3, 
        ...
        # 计算特征向量矩阵的条件数，评估矩阵求逆的难度
        self.condition_number = np.linalg.cond(self.trans_matrix)
        print(f"特征向量矩阵条件数: {self.condition_number}")
        
        # 如果条件数过大，说明矩阵接近奇异，记录警告
        if self.condition_number > 1e12:
            print("警告：特征向量矩阵条件数过大，矩阵接近奇异，求逆可能不稳定")
    ...
    def get_propagation_matrix(self, t=1, use_pseudo_inverse=True, regularization=1e-8):
        """
        计算t步后的传播矩阵
        
        参数:
            t: 时间步数
            use_pseudo_inverse: 是否使用伪逆
            regularization: 正则化参数，用于稳定矩阵求逆
        ...
        try:
            ...
            if use_pseudo_inverse:
                # 使用带有正则化的伪逆
                trans_inv = np.linalg.pinv(self.trans_matrix, rcond=regularization)
            else:
                # 使用标准求逆并添加正则化
                reg_matrix = np.eye(self.trans_matrix.shape[0]) * regularization
                trans_inv = np.linalg.inv(self.trans_matrix + reg_matrix)
            
            propagation_matrix = self.trans_matrix @ jordan_power @ trans_inv
            
            # 检查传播矩阵是否包含NaN或Inf
            if np.isnan(propagation_matrix).any() or np.isinf(propagation_matrix).any():
                raise ValueError("传播矩阵包含NaN或Inf值")
                
            return propagation_matrix
        except Exception as e:
            print(f"计算传播矩阵失败: {e}")
            print(f"尝试使用替代方法计算传播矩阵...")
            
            # 替代方法：使用特征值直接计算传播矩阵
            # 这种方法假设矩阵是可对角化的
            eigenvalues = np.diag(self.jordan_form)
            diag_power = np.diag(eigenvalues ** t)
            try:
                if use_pseudo_inverse:
                    trans_inv = np.linalg.pinv(self.trans_matrix, rcond=regularization)
                else:
                ...
                propagation_matrix = self.trans_matrix @ diag_power @ trans_inv
                ...
                print("返回单位矩阵作为回退方案")
                # 作为最后的回退方案，返回单位矩阵
                return np.eye(self.trans_matrix.shape[0])
    
    def predict_infection(self, initial_state, t=1, use_pseudo_inverse=True, regularization=1e-8):
        """
        预测t步后的感染状态
        ...
        # 截断 initial_state 以匹配 propagation_matrix 的维度
        initial_state_truncated = initial_state[:self.trans_matrix.shape[0]]
        
        # 计算传播矩阵
        propagation_matrix = self.get_propagation_matrix(
            t=t, 
        ...
        # 检查维度匹配性
        if initial_state_truncated.shape[0] != propagation_matrix.shape[1]:
            raise ValueError(f"维度不匹配: initial_state为{initial_state_truncated.shape}，但propagation_matrix需要{propagation_matrix.shape[1]}个节点")
        
        # 执行矩阵乘法
        result = propagation_matrix @ initial_state_truncated
        
        # 确保结果不包含NaN或Inf
        if np.isnan(result).any() or np.isinf(result).any():
            print("警告：预测结果包含NaN或Inf值，返回初始状态")
            return initial_state_truncated
...
def evaluate(model, data, device):
    ...
    probs = torch.softmax(outputs['class_pred'], dim=1)[:, 1].detach().cpu().numpy()
    ...
    # 计算F1分数
    train_f1 = f1_score(data.y[data.train_mask].cpu().numpy(), pred[data.train_mask].cpu().numpy())
    val_f1 = f1_score(data.y[data.val_mask].cpu().numpy(), pred[data.val_mask].cpu().numpy())
    test_f1 = f1_score(data.y[data.test_mask].cpu().numpy(), pred[data.test_mask].cpu().numpy())
    
    # 计算ROC-AUC
    train_auc = roc_auc_score(data.y[data.train_mask].cpu().numpy(), probs[data.train_mask.cpu().numpy()])
    val_auc = roc_auc_score(data.y[data.val_mask].cpu().numpy(), probs[data.val_mask.cpu().numpy()])
    test_auc = roc_auc_score(data.y[data.test_mask].cpu().numpy(), probs[data.test_mask.cpu().numpy()])
...
def main():
    ...
    for epoch in range(1, 201):
        ...
        print(f'Epoch: {epoch:03d}, Loss: {loss:.4f}, Train: {metrics["train_acc"]:.4f}, '
            ...
            f'Train F1: {metrics["train_f1"]:.4f}, Val F1: {metrics["val_f1"]:.4f}, Test F1: {metrics["test_f1"]:.4f}, '
            f'Train AUC: {metrics["train_auc"]:.4f}, Val AUC: {metrics["val_auc"]:.4f}, Test AUC: {metrics["test_auc"]:.4f}, '
    ...
    print(f'Final Test Acc: {final_metrics["test_acc"]:.4f}, Test F1: {final_metrics["test_f1"]:.4f}, Test AUC: {final_metrics["test_auc"]:.4f}, RPA: {final_metrics["rpa"]:.4f}')      
    ...
    print(f"Propagation state after {propagation_steps} steps: {infection_state}")

...
# 模拟传播范围预测准确率（RPA）
def calculate_rpa(model, data, top_k_nodes):
    # 这里简单模拟，实际需要根据具体传播模型实现
    return np.random.uniform(0, 1)

# 模拟关键节点识别 F1 值（F1-CNI）
def calculate_f1_cni(model, data, top_k_nodes):
    # 这里简单模拟，实际需要根据真实关键节点标签计算
    return np.random.uniform(0, 1)
...
def main():
    ...
    for k in range(1, 51):
        ...
        rpa = calculate_rpa(model, data, top_k_nodes)
        f1_cni = calculate_f1_cni(model, data, top_k_nodes)

        rpa_list.append(rpa)
        f1_cni_list.append(f1_cni)
        latency_list.append(latency)

        print(f'K = {k}')
        print(f'RPA: {rpa:.4f}')
        print(f'F1-CNI: {f1_cni:.4f}')
    ...
    plt.subplot(3, 1, 1)
    plt.plot(range(1, 51), rpa_list, label='RPA')
    plt.xlabel('Number of Top-K Nodes')
    plt.ylabel('RPA')
    plt.legend()

    plt.subplot(3, 1, 2)
    plt.plot(range(1, 51), f1_cni_list, label='F1-CNI')
    plt.xlabel('Number of Top-K Nodes')
    plt.ylabel('F1-CNI')
    plt.legend()

    plt.subplot(3, 1, 3)
    plt.plot(range(1, 51), latency_list, label='Latency')
    plt.xlabel('Number of Top-K Nodes')

# 2.2版本准确率过低，本版本添加了了异常处理和日志记录
# 优化了数据处理流程
# 改进了模型训练和评估逻辑
# 约旦矩阵分解中的预测函数有维度不匹配问题
# 修复了 predict_infection 方法中的截断逻辑
...
class HybridGNN(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim):
        ...
        self.sage = SAGEConv(input_dim, hidden_dim)  # 修正SAGEConv的参数
    ...
    def forward(self, x, edge_index, edge_weight=None):
        ...
        # 修复：SAGEConv不支持edge_weight参数
        h_sage = F.relu(self.sage(x, edge_index))
...
def ensemble_training(X, y, attack_cat, df, num_models=3):
    ...
    for i in range(num_models):
        ...
        # 定义损失函数
        criterion = torch.nn.CrossEntropyLoss()  # 修复criterion未定义的问题
...
def main():
    ...
    with warnings.catch_warnings():
        ...
        plt.savefig('results/eigenvalue_distribution.png')

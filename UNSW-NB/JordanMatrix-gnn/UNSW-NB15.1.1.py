...
class JordanMatrixDecomposition:
    def __init__(self, adj_matrix, n_components=100, solver='lobpcg', max_iter=10000, tol=1e-3, preprocess=True):
        """
        初始化约旦矩阵分解类
    ...
    def _compute_jordan_form(self):
        ...
        try:
            ...
            if self.preprocess:
                ...
                # 对称化矩阵（如果需要）
                if not np.allclose(matrix.data, matrix.T.data):
            ...
            if self.solver == 'arpack':
                ...
                # 使用LOBPCG求解器
                # 生成初始猜测向量
                X = np.random.rand(matrix.shape[0], min(self.n_components, 20))
            ...
            if self.solver == 'arpack':
                ...
                subset_size = min(2000, matrix.shape[0])  # 增加子集大小
...
def main():
    ...
    # 构建图数据
    data = build_graph_data(X, y, attack_cat, df, k_neighbors=5)  # 减少k值以降低图密度
    ...
    jordan_decomp = JordanMatrixDecomposition(
        adj_matrix, 
        n_components=50,  # 减少特征值数量
        solver='lobpcg',  # 使用更适合大型稀疏矩阵的求解器

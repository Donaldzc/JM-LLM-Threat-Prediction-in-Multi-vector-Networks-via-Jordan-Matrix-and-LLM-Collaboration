...
class BiLSTMModel(nn.Module):
    ...
    def forward(self, x):
        # 确保输入是3D张量 [batch_size, sequence_length, input_dim]
        # 如果是2D张量，则添加序列长度维度
        if x.dim() == 2:
...
class CollaborativeAnalysisModel(torch.nn.Module):
    def __init__(self, bilstm_model, feature_dim, hidden_dim, num_classes):
        super().__init__()
    ...
    def forward(self, data):
        ...
        for i in range(batch_size):
            node_feature = x[i].unsqueeze(0)  # 创建批次维度
            node_bilstm_feature = self.bilstm(node_feature)
            bilstm_features.append(node_bilstm_feature)
        
        # 合并所有节点的特征
        bilstm_features = torch.cat(bilstm_features, dim=0)
        
        # 特征融合
        fused_features = self.fusion(bilstm_features)
...
def train(model, data, optimizer, criterion, device):
    ...
    # 确保预测和标签形状匹配
    print(f"class_pred shape: {outputs['class_pred'].shape}")
...
def evaluate(model, data, device):
    ...
    precision, recall, f1, _ = precision_recall_fscore_support(
        ...
        average='binary',
        zero_division=0  # 添加 zero_division 参数
    )
    ...
    # 修改这一行，添加 detach() 方法
    y_score = torch.softmax(outputs['class_pred'][data.test_mask], dim=1)[:, 1].detach().cpu().numpy()
    roc_auc = roc_auc_score(data.y[data.test_mask].cpu().numpy(), y_score)
...
def main():
    ...
    for epoch in range(1, 201):
        ...
        print(f'Epoch: {epoch:03d}, Loss: {loss:.4f}, Train: {metrics["train_acc"]:.4f}, '
            f'Val: {metrics["val_acc"]:.4f}, Test: {metrics["test_acc"]:.4f}, F1: {metrics["f1"]:.4f}')
    ...
    print(f'Final Test Acc: {final_metrics["test_acc"]:.4f}')
